...
function Get-WorkflowFolder {
    ...
    $manifest = Get-ChildItem -Path $WorkflowsDir -Filter manifest.json -Recurse -File | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
    if (-not $manifest) {
        return $null
    }
...
$promptPath = Join-Path $workspaceDir "codex-$WorkflowType.prompt.md"
...
try {
    Write-Host ""
    Write-Host "==> Initializing Codex workspace"
    & node $binPath init codex
    if ($LASTEXITCODE -ne 0) {
        throw "init codex failed with exit code $LASTEXITCODE"
    }
...
"@
    ...
    Write-Host ""
    Write-Host "==> Running Codex $WorkflowType workflow"
    $prompt | codex exec --skip-git-repo-check --sandbox workspace-write --cd $workspaceDir -
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host ""
    Write-Host "Codex Tier 2 manual run passed."
