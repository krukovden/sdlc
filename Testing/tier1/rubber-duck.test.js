...
// ---------------------------------------------------------------------------
// Source contract tests — read directly from .agents/
// ---------------------------------------------------------------------------

describe('rubber-duck source contracts', () => {
  describe('sdlc-rubber-duck.md', () => {
    it('file exists in .agents/agents/', () => {
      const p = path.join(SDLC_ROOT, '.agents/agents/sdlc-rubber-duck.md');
      assert.ok(fs.existsSync(p), 'sdlc-rubber-duck.md not found in .agents/agents/');
    });

    it('has Role section', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('## Role'));
    });

    it('has Boundaries section', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('## Boundaries'));
    });

    it('has Verdict Format section', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('## Verdict Format'));
    });

    it('has Autonomous Pipeline Mode section', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('## Autonomous Pipeline Mode'));
    });

    it('declares cross-model intent', () => {
      const content = readSource('.agents/agents/sdlc-rubber-duck.md');
      assert.ok(
    ...
    it('declares terminal node', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('terminal node'));
    });

    it('declares independent retry budget', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('independent'));
    });

    it('enforces read-only boundary', () => {
      assert.ok(readSource('.agents/agents/sdlc-rubber-duck.md').includes('Do NOT modify code'));
    });
  ...
  describe('sdlc-lead.md', () => {
    it('includes Copilot Fleet as dispatch mode option', () => {
      assert.ok(readSource('.agents/agents/sdlc-lead.md').includes('Copilot Fleet'));
    });

    it('has Rubber Duck Model Selection section', () => {
      assert.ok(readSource('.agents/agents/sdlc-lead.md').includes('Rubber Duck Model Selection'));
    });

    it('includes Rubber Duck in pipeline sequence tables', () => {
      assert.ok(readSource('.agents/agents/sdlc-lead.md').includes('Rubber Duck'));
    });

    it('defines opposite-model selection logic', () => {
      const content = readSource('.agents/agents/sdlc-lead.md');
  ...
  describe('sdlc-security.md', () => {
    it('checks rubber_duck.enabled before deciding terminal status', () => {
      assert.ok(readSource('.agents/agents/sdlc-security.md').includes('rubber_duck.enabled'));
    });

    it('spawns Rubber Duck agent when enabled', () => {
      const content = readSource('.agents/agents/sdlc-security.md');
  ...
  describe('sdlc-plan SKILL.md', () => {
    it('has rubber_duck field in task format', () => {
      assert.ok(readSource('.agents/skills/sdlc-plan/SKILL.md').includes('rubber_duck'));
    });

    it('has Rubber Duck Evaluation section', () => {
      assert.ok(readSource('.agents/skills/sdlc-plan/SKILL.md').includes('Rubber Duck Evaluation'));
    });

    it('includes Rubber Duck Configuration in stop-gate', () => {
      assert.ok(readSource('.agents/skills/sdlc-plan/SKILL.md').includes('Rubber Duck Configuration'));
    });

    it('has enablement heuristics for auth/PII tasks', () => {
      const content = readSource('.agents/skills/sdlc-plan/SKILL.md');
  ...
  describe('sdlc-implement SKILL.md', () => {
    it('has three dispatch mode options', () => {
      const content = readSource('.agents/skills/sdlc-implement/SKILL.md');
    ...
    it('includes Rubber Duck in pipeline sequences', () => {
      assert.ok(readSource('.agents/skills/sdlc-implement/SKILL.md').includes('Rubber Duck'));
    });
...
describe('rubber-duck generated files', () => {
  ...
  it('claude agent references role instructions file', () => {
    assertFileContains(
      ...
      '.agents/agents/sdlc-rubber-duck.md',
    );
  });
});
